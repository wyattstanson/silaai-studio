...
/* Customer IDs: first three letters of the customer's name + "-" + a
   3-digit sequence, e.g. Anjali -> ANJ-001, Ravi -> RAV-001. */
export const CUSTOMER_ID_RE = /^[A-Z]{3}-\d{3}$/;
const custPrefix = (name: string) =>
  (name || "").replace(/[^a-zA-Z]/g, "").slice(0, 3).toUpperCase().padEnd(3, "X");
const makeCustomerId = (name: string, customers: Customer[]) => {
  const pre = custPrefix(name);
  const used = customers.filter(c => c.id.startsWith(pre + "-")).map(c => parseInt(c.id.slice(4), 10)).filter(n => !Number.isNaN(n));
  ...
  const store = useMemo<Store>(() => {
    ...
    return {
      ...
      customerSignup: (name, p) => {
        ...
        const cus: Customer = { id: makeCustomerId(name, db.customers), familyId: fam.id, name, phone: p, createdAt: new Date().toISOString(), measurements: [] };
        setDb(d => ({
          ...d,
      ...
      addCustomer: (c) => {
        ...
        const id = given && CUSTOMER_ID_RE.test(given) && !db.customers.some(x => x.id === given)
          ? given
          : makeCustomerId(rest.name, db.customers);
