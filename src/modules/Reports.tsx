...
import { STAGES, STAGE_META, KIND_LABEL } from "../lib/stages";
import "./modules.css";

const STAGE_COLOR: Record<string, string> = {
  new: "var(--text-faint)", cutting: "var(--info)", stitching: "var(--acacia)", ready: "var(--sage)", delivered: "var(--sage-deep)",
};
...
export function Reports() {
  ...
  const pipeline = orders.filter(o => o.stage !== "delivered").reduce((s, o) => s + o.price, 0);
  const outstanding = orders.reduce((s, o) => s + balance(o), 0);

  const byStage = STAGES.map(s => ({ key: s, label: STAGE_META[s].label, n: orders.filter(o => o.stage === s).length, color: STAGE_COLOR[s] }));
