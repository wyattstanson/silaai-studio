...
async function main() {
  ...
  await prisma.order.create({ data: { code: "S-45", customerId: ravi.id, kind: "WEDDING", garment: "Sherwani + Stole", materialSource: "SHOP", fulfilment: "LOCAL", design: "Cream brocade, self-work", material: "Brocade, cream", qty: 1, stage: "NEW", deadline: true, placedAt: day(-2), deliveryDate: day(1), price: 16500, remarks: "Groom's engagement, rush.", payments: { create: [{ kind: "ADVANCE", amount: 6000, method: "CARD", at: day(-2) }] } } });
  await prisma.order.create({ data: { code: "S-46", customerId: priya.id, kind: "STITCHING", garment: "Kurta Set", materialSource: "OUTSIDE", fulfilment: "LOCAL", design: "Straight cut, mandarin collar", material: "Cotton, indigo", qty: 3, stage: "DELIVERED", deadline: false, placedAt: day(-30), deliveryDate: day(-9), price: 3600, remarks: "Delivered & settled.", payments: { create: [{ kind: "ADVANCE", amount: 1500, method: "CASH", at: day(-30) }, { kind: "BALANCE", amount: 2100, method: "CASH", at: day(-9) }] } } });
